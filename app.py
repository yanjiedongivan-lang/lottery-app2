...
st.markdown("<p class='sub-header'>适配2月1日新规 | 新增福运奖检测 | 包含历史开奖查询</p>", unsafe_allow_html=True)

# 选项卡布局 (修复点：确保两个Tab都正常工作)
    ...
    if st.session_state['predictions']:
        ...
        for i, p in enumerate(st.session_state['predictions']):
            ...
            <div class="prediction-card" style="border-left-color: {border_color};">
                ...
                <div style="margin-top:8px; font-size:12px; color:#95a5a6; border-top:1px solid #eee; padding-top:5px;">
...
with tab_history:
    ...
    if not df_history.empty:
        ...
        st.dataframe(
            ...
            hide_index=True,
            column_config={
                "红球": st.column_config.TextColumn(width="large"),
                "蓝球": st.column_config.TextColumn(width="small"),
                "和值": st.column_config.NumberColumn(format="%d")
            }
        )
    else:
        st.warning("暂无历史数据")
    ...
    with col1:
        ...
        if 'data_loaded' in st.session_state:
            ...
            df_freq['号码'] = df_freq['号码'].apply(lambda x: f"No.{x}")
    ...
    with col2:
        ...
        if 'data_loaded' in st.session_state:
            ...
            df_omit['号码'] = df_omit['号码'].apply(lambda x: f"No.{x}")
